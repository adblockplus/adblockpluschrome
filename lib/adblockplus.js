  ...
  var ElemHide = exports.ElemHide =
  {
    ...
    _generateCSSContent: function()
    {
      ...
      for (var domain in domains)
      {
        ...
        if (domain)
        {
          _generatorResult13.push(("@-moz-document domain(\"" + domain.split(",").join("\"),domain(\"") + "\"){").replace(/[^\x01-\x7F]/g, escapeChar));
        }
        else
        {
        ...
        {
          _generatorResult13.push(selector.replace(/[^\x01-\x7F]/g, escapeChar) + "{" + cssTemplate.replace("%ID%", list[selector]) + "}");
        }
        _generatorResult13.push("}");
      }
      return _generatorResult13;
