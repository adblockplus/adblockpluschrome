...
require.scopes["filterClasses"] = (function()
{
  ...
  ActiveFilter.prototype =
  {
    ...
    isActiveOnlyOnDomain: function(docDomain)
    {
      ...
      if (this.ignoreTrailingDot) docDomain = docDomain.replace(/\.+$/, "");
      docDomain = docDomain.toUpperCase();
      for (var domain in (this.domains)) if (this.domains[domain] && domain != docDomain && (domain.length <= docDomain.length || domain.indexOf("." + docDomain) != domain.length - docDomain.length - 1)) return false;
      return true;
    },
    serialize: function(buffer)
    {
  ...
  RegExpFilter.fromText = (function(text)
  {
    ...
    if (match)
    {
      ...
      for (var _loopIndex2 = 0; _loopIndex2 < options.length; ++_loopIndex2)
      {
        ...
        else if (option == "SITEKEY" && typeof value != "undefined") siteKeys = value.split(/\|/);
      }
    }
    ...
    if (!blocking && siteKeys) contentType = RegExpFilter.typeMap.DOCUMENT;
    try
    {
      if (blocking) return new BlockingFilter(origText, text, contentType, matchCase, domains, thirdParty, collapse);
  ...
  {
    RegExpFilter.call(this, text, regexpSource, contentType, matchCase, domains, thirdParty);
    if (siteKeys != null) this.siteKeys = siteKeys;
  }
...
require.scopes["subscriptionClasses"] = (function()
{
  ...
  Subscription.prototype =
  {
    ...
    serialize: function(buffer)
    {
      ...
      if (this._fixedTitle) buffer.push("fixedTitle=true");
      if (this._disabled) buffer.push("disabled=true");
    },
  ...
  Subscription.fromObject = (function(obj)
  {
    ...
    try
    {
      ...
      if ("nextURL" in obj) result.nextURL = obj.nextURL;
      ...
      if ("softExpiration" in obj) result.softExpiration = parseInt(obj.softExpiration) || 0;
      if ("errors" in obj) result._errors = parseInt(obj.errors) || 0;
      if ("requiredVersion" in obj)
      {
        var addonVersion = require("info").addonVersion;
        result.requiredVersion = obj.requiredVersion;
        if (Services.vc.compare(result.requiredVersion, addonVersion) > 0) result.upgradeRequired = true;
      }
      if ("alternativeLocations" in obj) result.alternativeLocations = obj.alternativeLocations;
    ...
    catch (e)
    {
      if (!("title" in obj))
      {
        if (obj.url == "~wl~") obj.defaults = "whitelist";
        else if (obj.url == "~fl~") obj.defaults = "blocking";
        else if (obj.url == "~eh~") obj.defaults = "elemhide";
        if ("defaults" in obj)
        {
          var Utils = require("utils").Utils;
    ...
    if ("fixedTitle" in obj) result._fixedTitle = obj.fixedTitle == "true";
  ...
  SpecialSubscription.prototype =
  {
    ...
    isDefaultFor: function(filter)
    {
      if (this.defaults && this.defaults.length)
      {
        for (var _loopIndex6 = 0; _loopIndex6 < this.defaults.length; ++_loopIndex6)
        {
          ...
          if (filter instanceof(SpecialSubscription.defaultsMap[type])) return true;
          if (!filter instanceof ActiveFilter && type == "blacklist") return true;
        }
      }
      return false;
    },
    serialize: function(buffer)
    {
      ...
      if (this.defaults && this.defaults.length) buffer.push("defaults=" + this.defaults.filter(function(type)
      {
  ...
  SpecialSubscription.createForFilter = (function(filter)
  {
    ...
    {
      if (filter instanceof(SpecialSubscription.defaultsMap[type])) subscription.defaults = [type];
    }
    if (!subscription.defaults) subscription.defaults = ["blocking"];
    var Utils = require("utils").Utils;
  ...
  DownloadableSubscription.prototype =
  {
    ...
    serialize: function(buffer)
    {
      ...
      if (this.nextURL) buffer.push("nextURL=" + this.nextURL);
      ...
      if (this.softExpiration) buffer.push("softExpiration=" + this.softExpiration);
      if (this.errors) buffer.push("errors=" + this.errors);
      if (this.requiredVersion) buffer.push("requiredVersion=" + this.requiredVersion);
      if (this.alternativeLocations) buffer.push("alternativeLocations=" + this.alternativeLocations);
    }
  };
  return exports;
})();
require.scopes["filterStorage"] = (function()
{
  ...
  var FilterStorage = exports.FilterStorage =
  {
    ...
    get sourceFile()
    {
      ...
      if (!file) Cu.reportError("Adblock Plus: Failed to resolve filter file location from extensions.adblockplus.patternsfile preference");
    ...
    getGroupForFilter: function(filter)
    {
      ...
      for (var _loopIndex9 = 0; _loopIndex9 < FilterStorage.subscriptions.length; ++_loopIndex9)
      {
        ...
        {
          if (subscription.isDefaultFor(filter)) return subscription;
    ...
    {
      if (subscription.url in FilterStorage.knownSubscriptions) return;
      FilterStorage.subscriptions.push(subscription);
      FilterStorage.knownSubscriptions[subscription.url] = subscription;
      addSubscriptionFilters(subscription);
      if (!silent) FilterNotifier.triggerListeners("subscription.added", subscription);
    },
    removeSubscription: function(subscription, silent)
    {
      for (var i = 0; i < FilterStorage.subscriptions.length; i++)
      {
        if (FilterStorage.subscriptions[i].url == subscription.url)
        {
          ...
          if (!silent) FilterNotifier.triggerListeners("subscription.removed", subscription);
          return;
        }
      }
    },
    moveSubscription: function(subscription, insertBefore)
    {
      ...
      if (newPos < 0) newPos = FilterStorage.subscriptions.length;
    ...
    addFilter: function(filter, subscription, position, silent)
    {
      ...
      if (typeof position == "undefined") position = subscription.filters.length;
      if (filter.subscriptions.indexOf(subscription) < 0) filter.subscriptions.push(subscription);
      subscription.filters.splice(position, 0, filter);
      if (!silent) FilterNotifier.triggerListeners("filter.added", filter, subscription, position);
    },
          ...
          for (var j = positions.length - 1; j >= 0; j--)
          {
            ...
            if (subscription.filters[position] == filter)
            {
              ...
              if (subscription.filters.indexOf(filter) < 0)
              {
                ...
                if (index >= 0) filter.subscriptions.splice(index, 1);
              }
    ...
    {
      if (!subscription instanceof SpecialSubscription || subscription.filters[oldPosition] != filter) return;
    ...
    {
      if (!Prefs.savestats || PrivateBrowsing.enabled || !filter instanceof ActiveFilter) return;
      filter.hitCount++;
    ...
    resetHitCounts: function(filters)
    {
      ...
      {
        filters = [];
        for (var _loopIndex10 = 0; _loopIndex10 < Filter.knownFilters.length; ++_loopIndex10)
        {
          var filter = Filter.knownFilters[_loopIndex10];
          filters.push(filter);
        }
      }
      for (var _loopIndex11 = 0; _loopIndex11 < filters.length; ++_loopIndex11)
      {
        var filter = filters[_loopIndex11];
        filter.hitCount = 0;
        filter.lastHit = 0;
      }
    },
    ...
    loadFromDisk: function(sourceFile)
    {
      ...
      var readFile = (function(sourceFile, backupIndex)
      {
        ...
        IO.readFromFile(sourceFile, true, parser, (function(e)
        {
          ...
          if (e) Cu.reportError(e);
          if (e && !explicitFile)
          {
            sourceFile = this.sourceFile;
            if (sourceFile)
            {
              var _tempVar12 = /^(.*)(\.\w+)$/.exec(sourceFile.leafName) || [null, sourceFile.leafName, ""];
              var part1 = _tempVar12[1];
              var part2 = _tempVar12[2];
              sourceFile = sourceFile.clone();
              sourceFile.leafName = part1 + "-backup" + ++backupIndex + part2;
              IO.statFile(sourceFile, function(e, statData)
              {
                if (!e && statData.exists) readFile(sourceFile, backupIndex);
              });
              return;
            }
          }
          ...
          for (var i = 0; i < parser.subscriptions.length; i++)
          {
            ...
            if (subscription instanceof SpecialSubscription && subscription.filters.length == 0 && subscription.url in specialMap) parser.subscriptions.splice(i--, 1);
            else knownSubscriptions[subscription.url] = subscription;
          }
          ...
          if (sourceFile != this.sourceFile) this.saveToDisk();
      ...
      else
      {
        ...
        if (sourceFile) IO.statFile(sourceFile, callback);
        else callback(true);
      }
    },
    _generateFilterData: function(subscriptions)
    {
      var _generatorResult13 = [];
      _generatorResult13.push("# Adblock Plus preferences");
      _generatorResult13.push("version=" + formatVersion);
      var saved =
      {
        __proto__: null
      };
      ...
      for (var i = 0; i < subscriptions.length; i++)
      {
        ...
        for (var j = 0; j < subscription.filters.length; j++)
        {
          ...
          if (!(filter.text in saved))
          {
            ...
            for (var k = 0; k < buf.length; k++) _generatorResult13.push(buf[k]);
            buf.splice(0);
          }
        }
      }
    ...
    saveToDisk: function(targetFile)
    {
      ...
      var writeFilters = (function()
      {
        ...
        {
          if (!explicitFile) this._saving = false;
          if (e) Cu.reportError(e);
          ...
          else FilterNotifier.triggerListeners("save");
        }).bind(this), "FilterStorageWrite");
      }).bind(this);
      var checkBackupRequired = (function(callbackNotRequired, callbackRequired)
      {
        if (explicitFile || Prefs.patternsbackups <= 0) callbackNotRequired();
        else
        {
          IO.statFile(targetFile, function(e, statData)
          {
            if (e || !statData.exists) callbackNotRequired();
            else
            {
              var _tempVar14 = /^(.*)(\.\w+)$/.exec(targetFile.leafName) || [null, targetFile.leafName, ""];
              ...
              {
                if (!e && (!statData.exists || (Date.now() - statData.lastModified) / 3600000 >= Prefs.patternsbackupinterval)) callbackRequired(part1, part2);
                else callbackNotRequired();
              });
            }
          });
        }
      ...
      if (!explicitFile) this._saving = true;
      checkBackupRequired(writeFilters, removeLastBackup);
    },
    getBackupFiles: function()
    {
      var result = [];
      var _tempVar15 = /^(.*)(\.\w+)$/.exec(FilterStorage.sourceFile.leafName) || [null, FilterStorage.sourceFile.leafName, ""];
  ...
  {
    if (!(subscription.url in FilterStorage.knownSubscriptions)) return;
    for (var _loopIndex17 = 0; _loopIndex17 < subscription.filters.length; ++_loopIndex17)
    {
      var filter = subscription.filters[_loopIndex17];
      var i = filter.subscriptions.indexOf(subscription);
      if (i >= 0) filter.subscriptions.splice(i, 1);
    }
  }
  var PrivateBrowsing = exports.PrivateBrowsing =
  {
    ...
    observe: function(subject, topic, data)
    {
      ...
      {
        if (data == "enter") this.enabled = true;
        else if (data == "exit") this.enabled = false;
      }
    },
  ...
  INIParser.prototype =
  {
    ...
    process: function(val)
    {
      ...
      try
      {
        if (this.wantObj === true && (match = /^(\w+)=(.*)$/.exec(val))) this.curObj[match[1]] = match[2];
        else if (val === null || (match = /^\s*\[(.+)\]\s*$/.exec(val)))
        {
          if (this.curObj)
          {
            ...
            case "filter":
            case "pattern":
              if ("text" in this.curObj) Filter.fromObject(this.curObj);
              break;
            case "subscription":
            ...
            case "subscription patterns":
              if (this.subscriptions.length)
              {
                ...
                for (var _loopIndex18 = 0; _loopIndex18 < this.curObj.length; ++_loopIndex18)
                {
                  var text = this.curObj[_loopIndex18];
        ...
        else if (this.wantObj === false && val) this.curObj.push(val.replace(/\\\[/g, "["));
      }
      finally
      {
...
require.scopes["elemHide"] = (function()
{
  ...
  var ElemHide = exports.ElemHide =
  {
    ...
    init: function()
    {
      ...
      {
        if (name == "enabled") ElemHide.apply();
      });
      onShutdown.add(function()
      {
        ElemHide.unapply();
    ...
    add: function(filter)
    {
      ...
      {
        if (filter.text in knownExceptions) return;
        var selector = filter.selector;
        if (!(selector in exceptions)) exceptions[selector] = [];
        exceptions[selector].push(filter);
        knownExceptions[filter.text] = true;
      }
      else
      {
        if (filter.text in keyByFilter) return;
        var key;
        do
        {
    ...
    {
      var selector = filter.selector;
      if (!(filter.selector in exceptions)) return null;
      var list = exceptions[filter.selector];
      for (var i = list.length - 1; i >= 0; i--) if (list[i].isActiveOnDomain(docDomain)) return list[i];
      return null;
    },
    _applying: false,
    _needsApply: false,
    apply: function()
    {
      ...
      IO.writeToFile(styleURL.file, false, this._generateCSSContent(), (function(e)
      {
        ...
        if (e && e.result == Cr.NS_ERROR_NOT_AVAILABLE) IO.removeFile(styleURL.file, function(e2){});
        else if (e) Cu.reportError(e);
        if (this._needsApply)
        {
    ...
    _generateCSSContent: function()
    {
      ...
      for (var key in filterByKey)
      {
        ...
        var list;
        if (domain in domains) list = domains[domain];
        else
        {
          list =
          {
      ...
      if (!hasFilters) throw Cr.NS_ERROR_NOT_AVAILABLE
      ...
      for (var domain in domains)
      {
        ...
        if (domain) _generatorResult13.push(("@-moz-document domain(\"" + domain.split(",").join("\"),domain(\"") + "\"){").replace(/[^\x01-\x7F]/g, escapeChar));
        else
        {
          _generatorResult13.push("@-moz-document url-prefix(\"http://\"),url-prefix(\"https://\"),url-prefix(\"mailbox://\"),url-prefix(\"imap://\"),url-prefix(\"news://\"),url-prefix(\"snews://\"){");
        }
        for (var selector in list) _generatorResult13.push(selector.replace(/[^\x01-\x7F]/g, escapeChar) + "{" + cssTemplate.replace("%ID%", list[selector]) + "}");
        _generatorResult13.push("}");
      }
      return _generatorResult13;
    },
    ...
    getSelectorsForDomain: function(domain, specificOnly)
    {
      ...
      {
        var filter = filterByKey[key];
        if (specificOnly && (!filter.domains || filter.domains[""])) continue;
        if (filter.isActiveOnDomain(domain) && !this.getException(filter, domain)) result.push(filter.selector);
      }
      return result;
    }
  };
...
require.scopes["matcher"] = (function()
{
  ...
  var Filter = _tempVar19.Filter;
  var RegExpFilter = _tempVar19.RegExpFilter;
  var WhitelistFilter = _tempVar19.WhitelistFilter;
  ...
  Matcher.prototype =
  {
    ...
    findKeyword: function(filter)
    {
      ...
      if (Filter.regexpRegExp.test(text)) return defaultResult;
    ...
    _checkEntryMatch: function(keyword, location, contentType, docDomain, thirdParty)
    {
      ...
      {
        var filter = list[i];
        if (filter.matches(location, contentType, docDomain, thirdParty)) return filter;
      }
      return null;
    },
    matchesAny: function(location, contentType, docDomain, thirdParty)
    {
      ...
      if (contentType == "DONOTTRACK") candidates.unshift("donottrack");
  ...
  CombinedMatcher.prototype =
  {
    ...
    add: function(filter)
    {
      if (filter instanceof WhitelistFilter)
      {
        if (filter.siteKeys)
        {
          for (var i = 0; i < filter.siteKeys.length; i++) this.keys[filter.siteKeys[i]] = filter.text;
        }
        else this.whitelist.add(filter);
      }
    ...
    remove: function(filter)
    {
      if (filter instanceof WhitelistFilter)
      {
        if (filter.siteKeys)
        {
          for (var i = 0; i < filter.siteKeys.length; i++) delete this.keys[filter.siteKeys[i]];
        }
        else this.whitelist.remove(filter);
      }
    ...
    matchesAnyInternal: function(location, contentType, docDomain, thirdParty)
    {
      ...
      for (var i = 0, l = candidates.length; i < l; i++)
      {
        ...
        if (substr in this.blacklist.filterByKeyword && blacklistHit === null) blacklistHit = this.blacklist._checkEntryMatch(substr, location, contentType, docDomain, thirdParty);
      }
      return blacklistHit;
    },
    matchesAny: function(location, contentType, docDomain, thirdParty)
...
require.scopes["filterListener"] = (function()
{
  ...
  var ActiveFilter = _tempVar20.ActiveFilter;
  var RegExpFilter = _tempVar20.RegExpFilter;
  var ElemHideBase = _tempVar20.ElemHideBase;
  var Prefs = require("prefs").Prefs;
  var batchMode = false;
  var isDirty = 0;
  var FilterListener = exports.FilterListener =
  {
    ...
    setDirty: function(factor)
    {
      ...
      if (isDirty >= 1) FilterStorage.saveToDisk();
    }
  };
  ...
  function init()
  {
    FilterNotifier.addListener(function(action, item, newValue, oldValue)
    {
      ...
      if (match && match[1] == "filter") onFilterChange(match[2], item, newValue, oldValue);
      else if (match && match[1] == "subscription") onSubscriptionChange(match[2], item, newValue, oldValue);
      else onGenericChange(action, item);
    });
    var application = require("info").application;
    if (application == "chrome") flushElemHide = (function(){});
    else ElemHide.init();
  ...
  {
    if (!batchMode && ElemHide.isDirty) ElemHide.apply();
  }

  function addFilter(filter)
  {
    if (!filter instanceof ActiveFilter || filter.disabled) return;
    var hasEnabled = false;
    for (var i = 0; i < filter.subscriptions.length; i++) if (!filter.subscriptions[i].disabled) hasEnabled = true;
    if (!hasEnabled) return;
    if (filter instanceof RegExpFilter) defaultMatcher.add(filter);
    else if (filter instanceof ElemHideBase) ElemHide.add(filter);
  }
  ...
  function removeFilter(filter)
  {
    ...
    if (filter instanceof RegExpFilter) defaultMatcher.remove(filter);
    else if (filter instanceof ElemHideBase) ElemHide.remove(filter);
  }
  ...
  function onSubscriptionChange(action, subscription, newValue, oldValue)
  {
    ...
    if (action == "added" || action == "removed" || action == "disabled")
    {
      ...
      if (subscription.filters) subscription.filters.forEach(method);
    }
  ...
  {
    if (action == "hitCount" || action == "lastHit") FilterListener.setDirty(0.002);
    else FilterListener.setDirty(1);
    if (action != "added" && action != "removed" && action != "disabled") return;
    if ((action == "added" || action == "removed") && filter.disabled)
    {
      return;
    }
    if (action == "added" || action == "disabled" && newValue == false) addFilter(filter);
  ...
  function onGenericChange(action)
  {
    if (action == "load")
    {
      ...
      for (var _loopIndex21 = 0; _loopIndex21 < FilterStorage.subscriptions.length; ++_loopIndex21)
      {
        ...
        if (!subscription.disabled) subscription.filters.forEach(addFilter);
      }
      flushElemHide();
    }
    else if (action == "save") isDirty = 0;
  }
  return exports;
})();
require.scopes["synchronizer"] = (function()
{
  ...
  var CommentFilter = _tempVar22.CommentFilter;
  var _tempVar23 = require("subscriptionClasses");
  var Subscription = _tempVar23.Subscription;
  var DownloadableSubscription = _tempVar23.DownloadableSubscription;
    ...
    executeInternal: function(subscription, manual, forceDownload)
    {
      ...
      if (loadFrom == url)
      {
        if (subscription.alternativeLocations)
        {
          ...
          var totalWeight = 1;
          for (var _loopIndex24 = 0; _loopIndex24 < subscription.alternativeLocations.split(",").length; ++_loopIndex24)
          {
            var alternative = subscription.alternativeLocations.split(",")[_loopIndex24];
            if (!/^https?:\/\//.test(alternative)) continue;
            var weight = 1;
          ...
          for (var _loopIndex25 = 0; _loopIndex25 < options.length; ++_loopIndex25)
          {
            var _tempVar26 = options[_loopIndex25];
            var weight = _tempVar26[0];
            var alternative = _tempVar26[1];
            choice -= weight;
            if (choice < 0)
            {
      ...
      try
      {
        ...
        if ((request.channel) instanceof(Ci.nsIHttpChannel)) request.channel.redirectionLimit = 5;
        ...
        request.channel.notificationCallbacks =
        {
          ...
          getInterface: function(iid)
          {
            ...
            if (oldNotifications) return oldNotifications.QueryInterface(iid);
            else throw Cr.NS_ERROR_NO_INTERFACE
          },
          asyncOnChannelRedirect: function(oldChannel, newChannel, flags, callback)
          {
            ...
            if (flags & Ci.nsIChannelEventSink.REDIRECT_TEMPORARY) hadTemporaryRedirect = true;
            else if (!hadTemporaryRedirect) newURL = newChannel.URI.spec;
            if (oldEventSink) oldEventSink.asyncOnChannelRedirect(oldChannel, newChannel, flags, callback);
            else callback.onRedirectVerifyCallback(Cr.NS_OK);
          }
        };
      }
      catch (e)
      ...
      request.addEventListener("load", function(ev)
      {
        ...
        if (isBaseLocation && !hadTemporaryRedirect) subscription.alternativeLocations = request.getResponseHeader("X-Alternative-Locations");
        ...
        for (var _loopIndex27 = 0; _loopIndex27 < (newFilters || subscription.filters).length; ++_loopIndex27)
        {
          var filter = (newFilters || subscription.filters)[_loopIndex27];
          ...
          if (match)
          {
            ...
            if (match[2]) interval *= SECONDS_IN_HOUR;
        ...
        if (newFilters)
        {
          ...
          for (var i = 0; i < newFilters.length; i++)
          {
            ...
            if (match)
            {
              ...
              if (keyword == "redirect")
              {
                if (isBaseLocation && value != url) subscription.nextURL = value;
              }
              ...
              if (known) newFilters.splice(i--, 1);
            }
          }
        ...
        if (isBaseLocation && newURL && newURL != url)
        {
          ...
          if (listed) FilterStorage.removeSubscription(subscription);
          url = newURL;
          var newSubscription = Subscription.fromURL(url);
          for (var key in newSubscription) delete newSubscription[key];
          for (var key in subscription) newSubscription[key] = subscription[key];
          delete Subscription.knownSubscriptions[subscription.url];
          newSubscription.oldSubscription = subscription;
          subscription = newSubscription;
          subscription.url = url;
          if (!(subscription.url in FilterStorage.knownSubscriptions) && listed) FilterStorage.addSubscription(subscription);
        }
        if (newFilters) FilterStorage.updateSubscriptionFilters(subscription, newFilters);
  ...
  function checkSubscriptions()
  {
    ...
    for (var _loopIndex28 = 0; _loopIndex28 < FilterStorage.subscriptions.length; ++_loopIndex28)
    {
      ...
      if (subscription.softExpiration - time > MAX_EXPIRATION_INTERVAL) subscription.softExpiration = time + MAX_EXPIRATION_INTERVAL;
      if (subscription.softExpiration > time && subscription.expires > time) continue;
  ...
  function readFilters(subscription, text, errorCallback)
  {
    ...
    if (minVersion)
    {
      ...
      if (Services.vc.compare(minVersion, addonVersion) > 0) subscription.upgradeRequired = true;
    }
    lines.shift();
    var result = [];
    for (var _loopIndex29 = 0; _loopIndex29 < lines.length; ++_loopIndex29)
    {
      var line = lines[_loopIndex29];
      line = Filter.normalize(line);
      if (line) result.push(Filter.fromText(line));
    }
    return result;
  }
  ...
  {
    if (!isBaseLocation) subscription.alternativeLocations = null;
    try
    {
    ...
    if (!manual)
    {
      ...
      if (subscription.errors >= Prefs.subscriptions_fallbackerrors && /^https?:\/\//i.test(subscription.url))
      {
        ...
        request.addEventListener("load", function(ev)
        {
          ...
          if (match && match[1] == "301" && match[2]) subscription.nextURL = match[2];
