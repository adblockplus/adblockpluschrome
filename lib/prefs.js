...
let Prefs = exports.Prefs = {
  ...
  getDocLink(linkID)
  {
    return this.documentation_link.replace(/%LINK%/g, linkID)
                                  .replace(/%LANG%/g, info.locale);
  },
...
port.on("prefs.set", (message, sender) =>
{
  if (message.key == "notifications_ignoredcategories")
  {
    ...
    return Notification.toggleIgnoreCategory("*", !!message.value);
  }
...
port.on("prefs.toggle", (message, sender) =>
{
  ...
  {
    const {Notification} = require("../adblockpluscore/lib/notification");
    return Notification.toggleIgnoreCategory("*");
  }
