                  ...
                  updatesVersion > Prefs.last_updates_page_displayed)
  {
    ...
    }).then(() =>
    {
      if (!Prefs.suppress_first_run_page)
      {
        ...
        let url;
        if (firstRun || dataCorrupted || reinitialized)
          url = "first-run.html";
        else
          url = "updates.html";

        // Users with corrupted browser data will see this page each time their
        // browser starts. We avoid focusing the page for those users, in the
        // hope to make the situation less intrusive.
        let active = !(dataCorrupted || reinitialized);

        browser.tabs.create({active, url});
      }
    });
  }
}
