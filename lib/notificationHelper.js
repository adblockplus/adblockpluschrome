...
const {Matcher} = require("../adblockpluscore/lib/matcher");
...
const browserNotificationsSupported = info.platform != "edgehtml";
...
function matchesUrlFilter(url, {urlFilters})
{
  if (!(activeNotification.urlFilters instanceof Array))
    return true;
  ...
  return !!matcher.matchesAny(url, contentTypes.DOCUMENT, url.hostname);
}
...
function playNotificationIconAnimation(notification)
{
  let animateIcon = !(notification.urlFilters instanceof Array) &&
      matchesDisplayMethod("icon", notification);
  if (animateIcon)
  {
...
function notificationButtonClick(notificationId, buttonIndex)
{
  ...
  switch (button.type)
  {
    ...
    case "configure":
      ...
      break;
    case "question":
      notifications.triggerQuestionListeners(notificationId, buttonIndex == 0);
      notifications.markAsShown(notificationId);
      break;
  }
}
 ...
 * @property {string} [url]
 *   URL to check that the active notification matches.
 * @returns {?object}
 */
port.on("notifications.get", (message, sender) =>
{
  ...
  if (!matchesUrlFilter(message.url, activeNotification))
    return;
